edit before use script takes in unfiltered file 

#
# This script takes in filtered vcf file created by samtools after SNP calling and creates a master SNP list, it then uses each SNP as a point, it then looks down each of the 6 isolates 1000bp 
# It then outputs the number of SNPs it finds within that 1000bp window in each isolate 
# This script is for the filtered vcf file outputted from minion SNP calling using 6 c variants sequenced using illumina.
...
output_file_name = "/data/scratch/connellj/Fusarium_venenatum/MINion_SNP_Calling/sliding_window_filtered_SNP_calling_1000bp.csv"
...
def function_count_SNP_incidence (c_unique, all_c_varients, window_size):
	for SNP in c_unique:
		...
		for c_var_SNP in all_c_varients ["C1"]:
			if c_var_SNP <= SNP < (c_var_SNP + window_size):
...
with open (output_file_name, "wb") as file_out:
	header = "Unique_SNP_Position,C1,C2,C3,C4,C5,C6"
	file_out.write (header + "\n")
	for element in SNP_incidence:
		Unique_SNP_Position = str (element)
		...
		line = Unique_SNP_Position + "," + C1 + "," + C2 + "," + C3 + "," + C4 + "," + C5 + "," + C6
		file_out.write (line + "\n")